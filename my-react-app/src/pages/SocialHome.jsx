...
function SocialHome({ onLogout, currentUser }) {
  ...
  useEffect(() => {
    const h = (e) => {
      if (e.key === "Escape") setViewingUser(null);
    };
  ...
  const handleLike = useCallback(async (postId) => {
    try {
      ...
      setPosts((prev) =>
        prev.map((p) =>
          p._id === postId ? { ...p, likes: data.post.likes } : p,
        ),
      );
    } catch (err) {
      console.error(err);
  ...
  const submitComment = async (postId) => {
    ...
    try {
      ...
      setCommentState((prev) => ({
        ...prev,
        [postId]: { ...prev[postId], text: "", loading: false },
      }));
    } catch (err) {
      setCommentState((prev) => ({
        ...prev,
    ...
    <div className="sx-home">
      ...
      <nav className="sx-nav">
        <div className="sx-nav-inner">
          <div className="sx-logo" onClick={() => navigate("/home")}>
            SocialX
          </div>
      ...
      <div className="sx-layout">
        ...
        <aside className="sx-sidebar-left">
          ...
          {[
            ...
            <div
              key={path}
              className="sx-side-link"
              onClick={() => navigate(path)}
            >
              <span className="sx-side-icon">{icon}</span>
              {label}
            </div>
          ))}
          <div className="sx-side-divider" />
          <div className="sx-side-stats">
            <div className="sx-stat-item">
              <strong>{currentUser?.friends?.length || 0}</strong>
              <span>Friends</span>
            </div>
            <div className="sx-stat-item">
              <strong>
                {posts.filter((p) => p.author?._id === currentUser?._id).length}
              </strong>
              <span>Posts</span>
            </div>
          </div>
        ...
        <main className="sx-feed">
          ...
          <div className="sx-create-card">
            <div className="sx-create-row">
              ...
              <input
                ...
                onKeyDown={(e) =>
                  e.key === "Enter" && !e.shiftKey && handleCreatePost()
                }
              />
            </div>
            ...
            {postImagePrev && (
              <div className="sx-img-preview-wrap">
                <img
                  src={postImagePrev}
                  alt="preview"
                  className="sx-img-preview"
                />
                <button className="sx-img-remove" onClick={removeImage}>
                  ×
                </button>
              </div>
            )}
            ...
            <div className="sx-create-footer">
              ...
              {/* Visibility selector */}
              <select
                className="sx-visibility-select"
          ...
          {posts.map((post) => {
            ...
            const cs = commentState[post._id] || {
              open: false,
              text: "",
              loading: false,
            };
            ...
            return (
              <article key={post._id} className="sx-post">
                <div className="sx-post-head">
                  <div
                    className="sx-post-author-row"
                    onClick={() => author._id && openUserProfile(author)}
                  >
                    ...
                    <div>
                      <div className="sx-post-author-name">
                        {author.name || author.username}
                      </div>
                      <div className="sx-post-time">
                ...
                <div className="sx-post-actions-bar">
                  <button
                    className={`sx-act-btn${liked ? " sx-act-liked" : ""}`}
                    onClick={() => handleLike(post._id)}
                  >
                    {liked ? "❤️" : "🤍"} Like
                  </button>
                  <button
                    className="sx-act-btn"
                    onClick={() => toggleComments(post._id)}
                  >
                    💬 Comment
                  </button>
                ...
                {cs.open && (
                  <div className="sx-comments">
                      ...
                      return (
                        <div key={i} className="sx-cmt">
                          <div className="sx-cmt-av">
                            {cu.profilePic ? (
                              <img src={cu.profilePic} alt="" />
                            ) : (
                              <span>{getInitials(cu)}</span>
                            )}
                          </div>
                          <div className="sx-cmt-bubble">
                            <span className="sx-cmt-name">
                              {cu.name || cu.username || "User"}
                            </span>
                    ...
                    {!post.comments?.length && (
                      <p className="sx-no-cmt">No comments yet. Be first! 👇</p>
                    )}
                    ...
                    <div className="sx-cmt-input-row">
                      ...
                      <div className="sx-cmt-input-wrap">
                        <input
                          ...
                          onKeyDown={(e) =>
                            e.key === "Enter" && submitComment(post._id)
                          }
                        />
                        <button
                          className="sx-cmt-send"
                          onClick={() => submitComment(post._id)}
                          disabled={cs.loading || !cs.text?.trim()}
                        >
                          {cs.loading ? "…" : "↵"}
                        </button>
                      </div>
        ...
        <aside className="sx-sidebar-right">
          ...
          <div className="sx-widget">
            ...
            <p className="sx-widget-text">
              You can upload photos when creating a post. Images are stored
              securely.
            </p>
          </div>
            ...
            <div className="sx-modal-content">
              ...
              <div className="sx-modal-btns">
                {friendStatus === "friends" ? (
                  <>
                    <button
                      className="sx-mbtn sx-mbtn-primary"
                      onClick={handleMessage}
                    >
                      💬 Message
                    </button>
                    <button className="sx-mbtn sx-mbtn-secondary" disabled>
                      ✓ Friends
                    </button>
                  </>
                ) : friendStatus === "sent" ? (
                  ...
                  <>
                    ...
                    <button
                      className="sx-mbtn sx-mbtn-secondary"
                      onClick={handleMessage}
                    >
                      💬 Message
                    </button>
              ...
              <h4 className="sx-modal-posts-title">
